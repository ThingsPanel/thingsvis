...
function findEntry(pluginDir) {
  ...
  throw new Error(`[thingsvis][plugin-config] Cannot find plugin entry in ${pluginDir}. Expected one of: ${candidates.join(', ')}`);
}

/**
 * Create a standardized Rspack+MF2 config for ThingsVis plugins.
 *
 ...
 */
function createPluginConfig(pluginDir, opts = {}) {
  ...
  return defineConfig({
    ...
    optimization: {
      // 重要：为了支持离线/Blob加载 remoteEntry.js，MVP 阶段禁止插件拆分 chunk。
      splitChunks: false
    },
    plugins: [
