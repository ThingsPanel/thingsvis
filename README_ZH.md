...
**ThingsVis** 是一个工业级、低代码可视化平台，内置基于画布的编辑器，用于创建交互式可视化应用。它专为桌面控制台设计，提供强大的 Widget 生态系统和微内核架构，用于构建可扩展的可视化应用程序。

## 🌟 特性

- **画布编辑器**: 直观的拖拽式界面，用于创建可视化内容
- **Widget 生态系统**: 可扩展架构，支持多类型 Widget 组件分类体系
- **微内核设计**: 严格分离无 UI 的内核逻辑与可视化组件
- **Module Federation 2.0**: 支持动态 Widget 加载及离线缓存
...
- **Rspack + Module Federation 2.0** - 快速构建和动态 Widget 加载
- **React 18** - UI 渲染层
...
ThingsVis 提供了一个强大的 CLI 工具来快速生成新 Widget。
...
│   └── index.html        # 开发服务器着陆页
└── src/
    ├── index.ts          # Widget 主入口 (导出 Main 模块)
    └── spec.tsx          # 视觉隔离测试组件
```
...
# Widget 将在 http://localhost:<port>/remoteEntry.js 可用
```
...
将 Widget 添加到注册表文件 (`apps/studio/public/registry.json`):
...
每个 Widget 包含一个 `spec.tsx` 文件用于隔离视觉测试：

1. 启动 Widget 开发服务器: `pnpm dev`
2. 打开 studio 应用: `pnpm dev --filter ./apps/studio`
3. 从组件面板添加你的 Widget
4. 验证渲染和错误隔离
...
2. **Widget 隔离**: Widget 不能反向依赖 Kernel
...
1. **Basic** - 基础 UI 元素 (矩形, 文本, 圆形, 直线)
2. **Chart** - 数据可视化 (ECharts 折线图, 柱状图, 饼图)
3. **Media** - 图片、视频和媒体播放器
...
### v0.2.0 (即将发布)
- [ ] **Widget SDK v2** — `defineWidget()` 统一生命周期 API
- [ ] **实时协作** — 基于 CRDT 的多用户画布编辑
- [ ] **数据大屏模式** — 自动布局响应式网格，适配 IoT 监控大屏
- [ ] **MQTT 数据源** — 原生 MQTT 协议支持
- [ ] **条件可见性** — 将 Widget 可见性绑定到数据源值

### v0.3.0 (规划中)
- [ ] **移动端预览模式** — 响应式画布自适应移动视口
- [ ] **Widget 市场** — 社区 Widget 发布与安装
- [ ] **版本历史** — 基于快照的项目版本管理
- [ ] **访问控制** — 协作项目的角色权限管理

### 已完成 ✅
- [x] 基于画布的拖拽式编辑器
- [x] Module Federation 2.0 Widget 动态加载 + 离线缓存
- [x] WebSocket + REST 数据源（Auth/重连）
- [x] 撤销/重做命令历史
- [x] 嵌入模式（ThingsPanel 集成）
- [x] Widget SDK `defineWidget()` API
...
本项目基于 [Apache License 2.0](LICENSE) 开源授权。
