...
export class AutoSaveManager {
  ...
  init(
    ...
    this.config = {
      ...this.baseConfig,
      ...configOverride,
      mode: configOverride?.mode ?? this.baseConfig.mode,
    };
  ...
  markDirty(scheduleSave = true): void {
    ...
    if (!scheduleSave || this.config.mode !== 'auto') {
      return;
    }
  ...
  async saveNow(force = false): Promise<void> {
    ...
    this.updateStatus({
      ...this.currentState,
      status: 'error',
      error: `Save failed after ${maxAttempts} attempts: ${message}`,
    });
  }
