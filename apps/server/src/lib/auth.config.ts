  ...
  callbacks: {
    async jwt({ token, user }) {
      if (user) {
        ...
        token.authType = user.authType as string | undefined;
        token.loginSource = user.loginSource as string | undefined;
    ...
    async session({ session, token }) {
      if (session.user) {
        ...
        session.user.loginSource = token.loginSource as string | undefined;
