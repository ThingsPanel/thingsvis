...
- [x] T002 [P] Create shared plugins config file `F:/coding/thingsvis/configs/rspack-widget.config.js` (initial stub + exports)
...
- [x] T010 Export widget module types from `F:/coding/thingsvis/packages/thingsvis-schema/src/index.ts`
...
- [x] T016 [P] Add registry loader utility in `F:/coding/thingsvis/apps/preview/src/widgets/registryClient.ts` (fetch + validate with Zod; return typed map)
- [x] T017 Add widget resolver in `F:/coding/thingsvis/apps/preview/src/widgets/widgetResolver.ts` (given `componentId` → load `./Main` via `@thingsvis/kernel` UniversalLoader; handle errors)
...
- [x] T020 Implement “plugin renderer wrapper” in `F:/coding/thingsvis/packages/thingsvis-ui/src/engine/renderers/widgetRenderer.ts` (adapts `WidgetMainModule.create()` output into VisualEngine’s renderer interface)
...
**Checkpoint**: Foundation ready — the host can fetch/validate `registry.json`, load a widget `./Main` module, and VisualEngine can render different node types via pluggable renderers.
...
- [x] T022 [P] [US1] Create plugin package `F:/coding/thingsvis/widgets/basic/rect/package.json` (MF remote name, deps as peers/shared, scripts: dev/build/typecheck)
- [x] T023 [P] [US1] Create plugin config `F:/coding/thingsvis/widgets/basic/rect/rspack.config.js` extending `F:/coding/thingsvis/configs/rspack-widget.config.js`
- [x] T024 [P] [US1] Implement widget entry `F:/coding/thingsvis/widgets/basic/rect/src/index.ts` (export `componentId="basic/rect"`, `create()`, and re-export `Spec` from `src/spec.tsx`)
- [x] T025 [P] [US1] Implement visual test `F:/coding/thingsvis/widgets/basic/rect/src/spec.tsx` (renders rect in isolation using Leafer; no host dependencies)

- [x] T026 [P] [US1] Create plugin package `F:/coding/thingsvis/widgets/layout/text/package.json`
- [x] T027 [P] [US1] Create plugin config `F:/coding/thingsvis/widgets/layout/text/rspack.config.js` extending `F:/coding/thingsvis/configs/rspack-widget.config.js`
- [x] T028 [P] [US1] Implement widget entry `F:/coding/thingsvis/widgets/layout/text/src/index.ts` (export `componentId="layout/text"`, `create()`, and re-export `Spec`)
- [x] T029 [P] [US1] Implement visual test `F:/coding/thingsvis/widgets/layout/text/src/spec.tsx` (renders text in isolation using Leafer)

- [x] T030 [P] [US1] Create plugin package `F:/coding/thingsvis/widgets/media/image/package.json`
- [x] T031 [P] [US1] Create plugin config `F:/coding/thingsvis/widgets/media/image/rspack.config.js` extending `F:/coding/thingsvis/configs/rspack-widget.config.js`
- [x] T032 [P] [US1] Implement widget entry `F:/coding/thingsvis/widgets/media/image/src/index.ts` (export `componentId="media/image"`, `create()`, and re-export `Spec`)
- [x] T033 [P] [US1] Implement visual test `F:/coding/thingsvis/widgets/media/image/src/spec.tsx` (renders image in isolation using Leafer; uses a known local/remote sample image)
...
- [x] T035 [US1] Wire VisualEngine to load widget renderers via host resolver in `F:/coding/thingsvis/apps/preview/src/widgets/widgetResolver.ts` (resolve by `node.schemaRef.type`)
...
**Independent Test**: Load a widget once, then disable network in browser devtools and reload the page; previously used plugin still loads and renders.

### Implementation for User Story 2

- [x] T038 [US2] Add cache versioning checks in `F:/coding/thingsvis/apps/preview/src/widgets/registryClient.ts` (ensure `version` is passed into the loader calls)
...
- [x] T041 [US2] Add non-fatal error surfacing in `F:/coding/thingsvis/apps/preview/src/widgets/widgetResolver.ts` (set `node.error` + keep page usable when offline cache miss occurs)
...
**Independent Test**: Run CLI to generate a new plugin, add it to `registry.json`, start preview, and run the widget’s `Spec` successfully.
...
- [x] T048 [P] Ensure plugins do not bundle shared deps by verifying MF `shared` config in `F:/coding/thingsvis/configs/rspack-widget.config.js` (react/leafer/@thingsvis singletons)
...
- [x] T051 Create plugin package `F:/coding/thingsvis/widgets/custom/cyber-clock/package.json` (remote name `thingsvis-widget-custom-cyber-clock`, dev port 3104)
- [x] T052 Create plugin config `F:/coding/thingsvis/widgets/custom/cyber-clock/rspack.config.js` extending `F:/coding/thingsvis/configs/rspack-widget.config.js`
- [x] T053 Implement widget entry `F:/coding/thingsvis/widgets/custom/cyber-clock/src/index.ts` exporting `componentId="custom/cyber-clock"`, `create()`, and `Spec`
- [x] T054 Implement visual test `F:/coding/thingsvis/widgets/custom/cyber-clock/src/spec.tsx` (renders a cyber-style clock in isolation using Leafer)
