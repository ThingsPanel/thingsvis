...
```bash
pnpm add @thingsvis/widget-sdk
```

## 快速开始

### 方式一：defineWidget（推荐，傻瓜式）
...
```typescript
import { createControlPanel, generateControls } from '@thingsvis/widget-sdk';
...
```typescript
import { withMixins } from '@thingsvis/widget-sdk';
import { z } from 'zod';
...
```typescript
type DefineWidgetConfig<TProps> = {
  id: string;
  name: string;
  category?: 'basic' | 'chart' | 'interaction' | 'media' | 'data' | 'layout' | 'indicator' | 'geo' | 'custom';
  icon?: string;
  version?: string;
  schema: z.ZodObject<TProps>;
  controls?: SimpleGroupConfig | WidgetControls;
  enableAllBindings?: boolean;
  render: (el: HTMLElement, props: TProps, ctx: WidgetOverlayContext) => {
    update?: (props: TProps, ctx: WidgetOverlayContext) => void;
    destroy?: () => void;
  };
};
```
