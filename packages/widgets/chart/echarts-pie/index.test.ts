...
describe('chart/echarts-pie widget', () => {
  beforeEach(() => {
    setOption.mockClear();
    resize.mockClear();
    dispose.mockClear();
  ...
  it('renders sample data by default instead of a premature empty-state message', async () => {
    ...
    expect(Main.defaultProps.data).not.toHaveLength(0);
  ...
  it('shows localized empty guidance only after the data set is empty', async () => {
    ...
    expect(latestOption?.graphic?.style?.text).toBe('Add slices or bind a distribution data set');
