...
export const controls = createControlPanel()
  .addGroup('GaugeBasic', (b) => {
    b.addNumberInput('min', {
      label: { zh: '最小值', en: 'Minimum' },
      description: { zh: 'AQI 模式自动使用 0', en: 'AQI mode automatically uses 0' },
      binding,
    });
    b.addNumberInput('max', {
      label: { zh: '最大值', en: 'Maximum' },
      description: { zh: 'AQI 模式自动使用 500', en: 'AQI mode automatically uses 500' },
      binding,
    });
    b.addTextInput('unit', {
      label: { zh: '单位', en: 'Unit' },
      description: { zh: 'AQI 模式自动显示 AQI', en: 'AQI mode automatically displays AQI' },
      binding,
    });
  ...
  .addGroup('GaugeStyle', (b) => {
    b.addColorPicker('primaryColor', { label: { zh: '主题颜色', en: 'Primary Color' }, binding });
    b.addColorPicker('axisLabelColor', { label: { zh: '刻度颜色', en: 'Scale Color' }, binding });
    b.addColorPicker('detailColor', { label: { zh: '数值颜色', en: 'Value Color' }, binding });
