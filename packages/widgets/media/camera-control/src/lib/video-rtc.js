...
export class VideoRTC extends HTMLElement {
  ...
  /**
   * Play a direct HLS URL (MediaMTX, etc.) without go2rtc WebSocket.
   * @param {string} url
   */
  playDirectHls(url) {
    if (this.hls) {
    ...
    if (Hls.isSupported()) {
      ...
      this.hls.on(Hls.Events.ERROR, (_event, data) => {
        ...
        console.error('[VideoRTC] HLS error:', data);
        switch (data.type) {
  ...
  set src(value) {
    ...
    // Direct HLS playback for MediaMTX / standard .m3u8 URLs.
    if (value.match(/\.m3u8(\?.*)?$/i)) {
      this.wsURL = '';
      const playHls = () => {
        if (this.video) {
          this.playDirectHls(value);
        } else {
          setTimeout(playHls, 100);
        }
      };
      playHls();
      return;
  ...
  codecs(isSupported) {
    return this.CODECS.filter((codec) =>
      this.media.includes(codec.includes('vc1') ? 'video' : 'audio'),
    )
