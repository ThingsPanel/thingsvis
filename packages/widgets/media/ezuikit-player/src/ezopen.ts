...
export function parseCloudRecQuery(url: string): Partial<CloudRecOptions> {
  const trimmed = url.trim();
...
export function resolveCloudRecPlaybackParams(
  props: Props,
  beginOverride?: string,
): CloudRecOptions {
  ...
  const spaceId = resolveSpaceId(props);
  const busType = props.busType.trim() || parsed.busType || (spaceId ? '7' : '');
  const end =
    props.playbackEnd.trim() ||
    parsed.end ||
    (spaceId ? getDefaultRecEndTimestamp(begin) : '');
...
export function buildCloudRecEzopenUrl(
  source: EzopenSource,
  options: CloudRecOptions | string,
): string {
  const params: CloudRecOptions =
    typeof options === 'string' ? { begin: options } : options;
  const spaceId = params.spaceId?.trim();
  const busType = params.busType?.trim() || (spaceId ? '7' : '');
  const end =
    params.end?.trim() ||
    (spaceId && params.begin ? getDefaultRecEndTimestamp(params.begin) : '');

  // Cloud recording (busType=7) uses SD cloud.rec per EZUIKit cloudRecord.md.
  const stream = spaceId
    ? `${source.channel}.cloud.rec`
    : source.hd
      ? `${source.channel}.hd.cloud.rec`
      : `${source.channel}.cloud.rec`;
  ...
  if (busType) query.set('busType', busType);
