...
export * from './platform-field-config';
export * from './field-mapping';
// NOTE: config-v2 is NOT re-exported here to avoid circular dependency.
// It is exported from the package root: @thingsvis/schema (src/index.ts).
...
import { PlatformFieldConfigSchema, DEFAULT_PLATFORM_FIELD_CONFIG } from './platform-field-config';
...
/**
 * Trigger mode for data sources:
 * - 'auto': connect + fetch on page load, enable polling (for telemetry / read endpoints)
 * - 'manual': register adapter only, no fetch/polling on load (for control / write endpoints)
 */
