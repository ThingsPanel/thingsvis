...
/**
 * Legacy MVP store types — extended locally for backward compat.
 * This entire file is an MVP stub; the real store lives in ./slices/.
 * @deprecated — Will be removed once all consumers migrate to the Zustand store.
 */
...
export const action = {
  ...
  groupNodes(pageId: string, nodeIds: string[], groupId?: string) {
    ...
    emitter.emit(
      'patches',
      nodeIds.map((id) => ({ nodeId: id, patch: { groupId: gid } }) as Patch),
    );
    return { groupId: gid };
  },
