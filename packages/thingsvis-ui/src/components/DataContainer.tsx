 ...
 * Platform data is accessed via ds.__platform__ in the kernel store.
 */
export const DataContainer: React.FC<DataContainerProps> = ({ store, nodeId, children }) => {
  ...
  const resolvedProps = useMemo(() => {
    if (!node) return {};
    const variableValues = (kernelState as KernelState & { variableValues?: Record<string, unknown> }).variableValues;
    return PropertyResolver.resolve(node, kernelState.dataSources as Record<string, unknown>, variableValues);
  }, [node, kernelState.dataSources, (kernelState as KernelState & { variableValues?: Record<string, unknown> }).variableValues]);
