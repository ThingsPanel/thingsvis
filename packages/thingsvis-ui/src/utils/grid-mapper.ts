...
import { DEFAULT_BREAKPOINTS } from '@thingsvis/schema';
...
export function getActiveBreakpoint(
  ...
  // Fall back to default breakpoints when none are configured
  const effectiveBreakpoints = (breakpoints && breakpoints.length > 0) ? breakpoints : DEFAULT_BREAKPOINTS;

  // Sort by minWidth descending
  const sorted = [...effectiveBreakpoints].sort((a, b) => b.minWidth - a.minWidth);

  // Find the first (largest) breakpoint where containerWidth >= minWidth
...
export function getEffectiveCols(
  ...
  // Always apply breakpoint logic when responsive=true (DEFAULT_BREAKPOINTS used as fallback inside getActiveBreakpoint)
